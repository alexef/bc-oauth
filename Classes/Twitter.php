...
class BC_OAuth_Twitter {
	...
	function __construct(){
		...
		$this->oauth_enabled = get_site_option('t_oauth_enabled');
		$this->consumer_key = get_site_option('t_oauth_consumer_key');
		$this->consumer_secret = get_site_option('t_oauth_consumer_secret');
		$this->oauth_host = get_site_option('t_server_uri');
		
		$this->call_back = site_url() . '/wp-load.php?action=bc_oauth&service=twitter';
	...
	function auth(){
		...
		try {
			...
			if (empty($_GET["oauth_token"])) {
				...
				$request = new OAuthRequester("http://api.twitter.com/1/account/verify_credentials.json", 'GET', $tokenResultParams);
				...
				if ($result['code'] == 200) {
					...
					if( $user->name && $user->id ){
						...
						if( !$create_user->check_user( false, array('bc_oauth_twitter_id' => $id) ) ){
							$create_user->bc_oauth_create_user($name, false, false, false, false, false, false, false, $name, array('bc_oauth_twitter_id' => $id) );
						} else {
